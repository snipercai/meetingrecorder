...
| LLM_MODE | LLM模式（local/api） | api | LLM_MODE |
...
LLM总结模块负责对会议转写内容进行定时总结，支持本地模型和OpenAI兼容API两种模式。
...
| transformers | 本地LLM模型（可选） | 4.28+ |
| torch | 深度学习框架（可选） | 1.13+ |
...
3. **智能总结**：利用LLM模型生成高质量会议总结
4. **灵活性**：支持本地模型和API调用两种模式
